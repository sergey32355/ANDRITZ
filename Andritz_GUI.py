...
from empa_gui import Ui_EmpaGUI
...
class MainWindow(QMainWindow):
    ...
    def Tool_Show_Spectrograms_Botton_Click(self):
        ...
        for i in range(start_,end_):
            # Define transform            
            waveform=torch.tensor(self.plates[indx_plate].segments_sign[indx_segment][i])
            if(len(waveform.shape)==1): waveform = waveform[None, :]
            if(spec_type=="MEL"):
                    spectrogrym_MEL_nfft=int(self.proc_settings.get("spectrogrym_MEL_nfft"))
                    spectrogram = F.Spectrogram(n_fft=spectrogrym_MEL_nfft)
                    # Perform transform
                    ...
                    if os.path.exists(f_name)==True:
                        ...
                        while(True):
                            ...
                            if (os.path.exists(f_name)==False):
                                break
                    arr=self.plates[indx_plate].segments_sign[indx_segment][k]
                    df = pd.DataFrame(arr)
                    df.to_csv(f_name)
    ...
    def Classification_Set_Category_Click(self):
        ...
        try:
            ...
            if(label=="" or label==" "):
                ...
                return
            #self.plates[indx_plate].segments_labels[indx_segment].append(list([start_el,end_el,label]))
    ...
    def Classification_All_Segm_To_Label_Click(self):
        ...
        print("label assigned to:")
        print("plate name - "+str(p_name))
        print("segment num. - "+str(len(self.plates[indx_plate].segments_sign)))       
    ...
    def Classification_ClearLabelling_Click(self):
        ...
        segment_name=self.ui.plot_segment_dropdown.currentText()    
        indx_plate,indx_chan, indx_segment = shlp.FindPlateInArray(plates=self.plates.copy(),plate_name=p_name,chan_name=ch_name,segm_name=segment_name)
        self.plates[indx_plate].segments_labels[indx_segment]=[]
    ...
    def Classification_ClearLabellingForAllSegments_Click(self):
        ...
        segment_name=self.ui.plot_segment_dropdown.currentText()   
        indx_plate,indx_chan, indx_segment = shlp.FindPlateInArray(plates=self.plates.copy(),plate_name=p_name,chan_name=ch_name,segm_name=segment_name)
        l_sl=len(self.plates[indx_plate].segments_labels)
        for jk in range(0,l_sl):
            self.plates[indx_plate].segments_labels[jk]=[]
        if plt.fignum_exists(self.classif_plot_fig_id)==True:
            self.Classification_Plot_Segment()
        print("")
    ...
    def RunClassificationClick(self):
        ...
        try:
            ...
            if(plate_segm == "this_plate_all_segments"):
                print("plate name - "+str(p_name))
                print("egm.num - "+str(len(self.plates[indx_plate].segments_sign)))
    ...
    def SaveLabelingAsCSV_Click(self):
        ...
        for l in range(0,len(self.plates[indx_plate].segments_labels[indx_segment])):
            #save labels here
            label=int(self.plates[indx_plate].segments_labels[indx_segment][l][2])
            ...
            for k in range(start_,end_):             
                    ...
                    if os.path.exists(f_name)==True:
                        ...
                        while(True):
                            ...
                            if (os.path.exists(f_name)==False):
                                break
                    st1=int(self.plates[indx_plate].segments_labels[indx_segment][l][0])            
                    st2=int(self.plates[indx_plate].segments_labels[indx_segment][l][1])            
                    arr=self.plates[indx_plate].segments_sign[indx_segment][k][st1:st2]
                    df = pd.DataFrame(arr)
                    df.to_csv(f_name)
    ...
    def SaveAllPlatesSegmToFile_Click(self):
        ...
        for p in range(0,len(self.plates)):
            ...
            l_sl=len(plate.segments_labels)
            for jk in range(0,l_sl):
                plate.segments_labels[jk]=[]
            #assign fake label to all plates segments
            plate.AssignLabelToAllSegments(label=0)

            for b in range(0,len(plate.segments_sign)):

                signal=plate.segments_sign[b]
    ...
    def SearchBestParams1(self):
        ...
        for kks in range(0,range_):
            ...
            for isk in range(0,len(self.plates)):#tqdm(range(len(self.plates)),desc="Plates analysis for snip.size "+str(cur_snip_size)):
                ...
                for b in tqdm(range(0,len(plate.segments_sign)),desc="Progress: plate "+str(isk)+" out of "+str(len(self.plates))):
                ...
                empty_plates_segm_cnt.append(empty_segments_cnt)#print("Empty segm.: "+str(empty_segments_cnt)+" out of "+str(len(plate.segments_sign))+" total segm. count")
    ...
    def RunClassifForSegment_Click(self):
        ...
        if(self.proc_settings.get("plate_segm_process")=="this_plate_this_segment"):
            ...
            fd=dp.SplitEntireSignalIntoSnippets(signal=self.plates[indx_plate].segments_sign[indx_segment],
            ...
            sgn=self.plates[indx_plate].segments_sign[indx_segment][indx_chan] 
            
            labels_in_segment=[]
            for l in range(0,len(self.plates[indx_plate].segments_sign)):
        ...
        if(self.proc_settings.get("plate_segm_process")=="this_plate_all_segments"):
            ...
            segm_num=len(self.plates[indx_plate].segments_sign)
            ...
            for n_segm in range(0,segm_num):
                ...
                fd=dp.SplitEntireSignalIntoSnippets(signal=self.plates[indx_plate].segments_sign[n_segm],
                                            channs_indx=CHANNELS_TO_USE,
                                            torch_tensor=False,
                                            snip_size = snip_size,
                                            preproc_type = self.proc_settings.get("classification_preproc_dropdown")
    ...
    def SaveLabelingToFile_Click(self):
        ...
        for plate in self.plates:
            p_name=plate.name
            for s_indx in range(0,len(plate.segments_sign)):
                s_name=plate.segments_names[s_indx]
                for label in plate.segments_labels[s_indx]:
                    l_data=label
    ...
    def LoadLabelingFromFile_Click(self):
        ...
        for k in range(0,len(df)):
            ...
            #assign label
            self.plates[indx_plate].segments_labels[indx_segment].append(l_data)
        print("")
    ...
    def Real_Time_Button_Click(self):    
        ...
        self.rt_data_proc=shlp.DataPreproc(
            n_fft=int(self.proc_settings.get("spectrogrym_MEL_nfft")),
            n_mels=int(self.proc_settings.get("Settings_MEL_num_MELS_2")),
            n_mfcc=int(self.proc_settings.get("Settings_nmfcc_num_MFCC_text")),
        )
    ...
    def Spectrum_card_tracking(self):
        ...
        # Goal: wait for the trigger, then record it for a fixed amount of time, and repeat for the number of times indicated.
        ...
        with spcm.Card(card_type=spcm.SPCM_TYPE_AI) as card:
            ...
            while(getattr(t, "do_run", True)):
                ...
                with warnings.catch_warnings():
                    ...
                    for i in range(card.num_channels()):
                ...
                with open(f"data/RT/data_{dt}.pkl", "wb") as file:
    ...
    def ShowProcResults_REAL_TIME(self,plate,labels_in_segment):
        ...
        if(isinstance(self.RT_fig_proc_results,RTPlotWidget_1)):#,pg.widgets.PlotWidget.PlotWidget)):
        ...
        if(mark_segm_borders==True):           
                if(isinstance(self.RT_fig_proc_results,RTPlotWidget_1)):
        ...
        if (labels_in_segment is not None) and (len(labels_in_segment)!=0):                           
            ...
            for p in range(0,len(labels_in_segment)):                
                if(p>0): cnt=cnt+len(plate.segments_sign[p-1][chan_n])   
        ...
        elif(isinstance(self.RT_fig_proc_results,RTPlotWidget_1)):        
        ...
        while(getattr(t, "do_run", True)):
            ...
            for l in range(0,len(txtfiles)):
                ...
                if(Proceed_to_proc==True):
                    sgn_len = len(plates[0].segments_sign)
                    all_labels=[]
                    all_segm_sign=[]
                    for seg_i in range(0,sgn_len):
                        #take segment
                        segm_signal=plates[0].segments_sign[seg_i]
...
class WorkerRTShow(PySide6.QtCore.QObject):
    ...
    def __init__(self,parent,plate,labels_in_segment,curMeas_num,chans_num):
            ...
            if(isinstance(self.parent.RT_fig_proc_results,RTPlotWidget_1)):#,pg.widgets.PlotWidget.PlotWidget)):
                #https://pyqtgraph.readthedocs.io/en/latest/getting_started/plotting.html            
    ...
    def run(self):
            ...
            if(mark_segm_borders==True):           
                    if(isinstance(self.parent.RT_fig_proc_results,RTPlotWidget_1)):
